...
void Sample::Init()
{
	...
	auto asset = std::make_shared<TAssetFileFormat>();
	if (m_FbxImporter.Load(list[0], asset.get()))
	{
		auto exportFileName = L"../../data/asset/SKM_Manny.asset";
		TAssetFileFormat::Export(asset.get(), exportFileName);
	}
	...
	//	if (TAssetFileFormat::Import(list[iObj], fbxobj))
	//	{
	//		// 몇개의 오브젝트가 있느냐?
