...
void TSkyObject::PostRender()
{
	auto matView = TEngine::g_pCamera->m_matView;
	matView._41 = 0.0f;
	matView._42 = 0.0f;
	matView._43 = 0.0f;
	m_cbData.matView = 
	...
	TDevice::m_pd3dContext->PSSetSamplers(0, 1, 
		TDxState::m_pPointSS.GetAddressOf());
	...
	TDevice::m_pd3dContext->RSSetState(TDxState::m_pRSSolid.Get());
	TDevice::m_pd3dContext->PSSetSamplers(0, 1, TDxState::m_pLinearSS.GetAddressOf());
	if (TDevice::m_DepthEnable)
	{
		TDevice::m_pd3dContext->OMSetDepthStencilState(TDxState::m_pDSSDepthEnable.Get(), 0);
	}
	else
	{
		TDevice::m_pd3dContext->OMSetDepthStencilState(TDxState::m_pDSSDepthDisable.Get(), 0);
	}
	if (!TDevice::m_bWireFrame)
	{
		TDevice::m_pd3dContext->RSSetState(TDxState::m_pRSSolid.Get());
	}
	else
	{
		TDevice::m_pd3dContext->RSSetState(TDxState::m_pRSWireFrame.Get());
	}
}